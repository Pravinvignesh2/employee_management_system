    ...
    <div class="leave-container">
      ...
      <div class="records-section">
        <div class="section-header">
          ...
          <div class="filters" *ngIf="isAdmin"> <!-- admin only -->
          ...
          <div class="filters" *ngIf="!isAdmin && isAdminOrManager"> <!-- manager only -->
            <select [(ngModel)]="selectedStatus" (change)="loadLeaveRecords()" class="status-filter">
              <option value="">All Status</option>
              <option value="PENDING">Pending</option>
              <option value="APPROVED">Approved</option>
        ...
        <div class="modal-content" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body" *ngIf="selectedLeaveForDetails">
            ...
            <div class="detail-row">
              ...
              <span>{{ selectedLeaveForDetails.department }}</span>
            </div>
...
export class LeaveManagementComponent implements OnInit {
  ...
  loadLeaveRecords(): void {
    ...
    // Manager: use department-specific endpoint (same pattern as employee directory)
    if (!this.isAdmin && this.isAdminOrManager && this.currentUser?.department) {
      this.leaveService.getLeavesByDepartment(this.currentUser.department).subscribe(
        records => {
          this.leaveRecords = records;
          this.totalPages = 1;
          this.isLoading = false;
        ...
        error => {
          console.error('Error loading department leave records:', error);
          this.isLoading = false;
        }
      );
      return;
    ...
    // Admin: use server-side filtering
    const filters: LeaveFilters = {};
    if (this.selectedStatus) filters.status = this.selectedStatus as LeaveStatus;
    if (this.selectedLeaveType) filters.leaveType = this.selectedLeaveType as LeaveType;
  ...
  canApproveLeave(leave: Leave): boolean {
    ...
    if (leave.userId === this.currentUser.id) {
      return false;
    }

    // Get the leave requester's role and department
    const leaveRequesterRole = this.getLeaveRequesterRole(leave);
    const leaveRequesterDepartment = leave.department;

    // Manager's leave: only Admin can approve
    if (leaveRequesterRole === 'MANAGER') {
      return this.isAdmin;
    }

    // Admin's leave: only another Admin can approve
    if (leaveRequesterRole === 'ADMIN') {
      return this.isAdmin;
    }

    // Employee's leave: only Manager of same department can approve
    if (leaveRequesterRole === 'EMPLOYEE') {
      if (this.authService.hasRole('MANAGER')) {
        return this.currentUser.department === leaveRequesterDepartment;
      }
    }
  ...
  // Helper method to get leave requester's role
  private getLeaveRequesterRole(leave: Leave): string {
    return leave.userRole || 'EMPLOYEE'; // Use actual role from leave data
  }

  canCancelLeave(leave: Leave): boolean {
    // Only owner can cancel (no admin override)
    return (leave.status === LeaveStatus.PENDING || leave.status === LeaveStatus.APPROVED) &&
           (leave.userId === this.currentUser.id);
  }

  canDeleteLeave(leave: Leave): boolean {
    // Employees can delete their own pending leaves; Admin can delete any
    if (this.isAdmin) return true;
    return leave.status === LeaveStatus.PENDING && leave.userId === this.currentUser.id;
  }
